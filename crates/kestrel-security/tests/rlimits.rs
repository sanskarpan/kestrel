...
fn test_apply_rlimits_lowers_nofile_limit() {
    kestrel_ns::test_util::run_isolated(|| {
        ...
        assert!(
            lower_soft < current.0,
            "test assumes RLIMIT_NOFILE's current soft limit is > 0"
        );
...
fn test_set_oom_score_adj_writes_own_proc_file() {
    kestrel_ns::test_util::run_isolated(|| {
        set_oom_score_adj(500)
            .expect("setting your own oom_score_adj upward from the default needs no privilege");
