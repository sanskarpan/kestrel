...
fn test_scan_copy_ups_against_a_real_kernel_triggered_copy_up() {
    ...
    common::run_in_fresh_mount_ns(move || {
        ...
        fs::write(
            base_diff.join("big.txt"),
            b"original-content-from-base-layer",
        )
        .unwrap();

        let snapshotter = Snapshotter::new(data_dir.clone(), false);
        let snap = snapshotter
            .prepare_snapshot("c-copyup-1", &["sha256:base".into()])
            .unwrap();
        ...
        assert!(
            ...
            "the whole file, not just the written byte, should have been copied up"
        );
...
fn test_scan_copy_ups_detects_a_real_kernel_written_whiteout() {
    ...
    common::run_in_fresh_mount_ns(move || {
        ...
        let snapshotter = Snapshotter::new(data_dir.clone(), false);
        let snap = snapshotter
            .prepare_snapshot("c-copyup-2", &["sha256:base".into()])
            .unwrap();
        mount_overlay(&data_dir, &snap, false, false, false).expect("mount_overlay");
        ...
        assert_eq!(
            events[0].path,
            std::path::Path::new("deleted-in-container.txt")
        );
